...
func getTime(timeStr string) string {
	r1 := regexp.MustCompile(`(\d{2})-(\w+)-(\d{0,4}) (\d{2}:\d{2}:\d{2})`)
	if r1.MatchString(timeStr) {
		match := r1.FindStringSubmatch(timeStr)
		testStr := fmt.Sprintf("%s-%s-%sT%s+00:00", match[3], returnMonth(match[2]), match[1], match[4])
		return testStr
	}
	return ""
}

func returnMonth(month string) string {
	if month == "Jan" {
		return "01"
	}
	if month == "Feb" {
...
func (c *Tail) Run() {
	...
	for line := range c.tailf.Lines {
		...
		if timestampIndex != -1 {
			t, err = time.Parse(time.RFC3339, getTime(matches[timestampIndex]))
			if err != nil {
				continue
			}
		} else {
